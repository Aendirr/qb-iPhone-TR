...
RegisterCommand('phone', function()
    ...
    if not PhoneData.isOpen and LocalPlayer.state.isLoggedIn then
        if not PlayerData.metadata['ishandcuffed'] and not PlayerData.metadata['inlaststand'] and not PlayerData.metadata['isdead'] and not IsPauseMenuActive() then
            ...
            QBCore.Functions.Notify("İşlem şu anda mevcut değil..", "error")
        end
    end
end)
...
RegisterNUICallback('track-vehicle', function(data, cb)
    ...
    if findVehFromPlateAndLocate(veh.plate) then
        QBCore.Functions.Notify("Aracınız İşaretlendi.", "success")
    else
        QBCore.Functions.Notify("Araçınız İşaretlenemedi.", "error")
    end
    cb("ok")
end)
...
RegisterNUICallback('SetAlertWaypoint', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS Konum ayarı: '..data.alert.title)
...
RegisterNUICallback('RaceDistanceCheck', function(data, cb)
    QBCore.Functions.TriggerCallback('qb-lapraces:server:GetRacingData', function(RaceData)
        ...
        if dist <= 115.0 then
            ...
            QBCore.Functions.Notify('Yarıştan çok uzaktasın. GPS yarışa ayarlandı.', 'error', 5000)
...
RegisterNUICallback('SetHouseLocation', function(data, cb)
    ...
    QBCore.Functions.Notify("GPS ayarlandı " .. data.HouseData.HouseData.adress .. "!", "success")
    cb("ok")
end)
...
RegisterNUICallback('SetGPSLocation', function(data, cb)
    ...
    QBCore.Functions.Notify('GPS ayarlandı!', 'success')
    cb("ok")
end)
...
RegisterNUICallback('CanTransferMoney', function(data, cb)
    ...
    if (PlayerData.money.bank - amount) >= 0 then
		...
		SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { timeout=3000, title = "Bank", text = "Hesap Mevcut Değil!", icon = "fas fa-university", color = "#ff0000", }, })
...
RegisterCommand('ping', function(_, args)
    if not args[1] then
        QBCore.Functions.Notify("Bir Oyuncu Kimliği girmeniz gerekiyor", "error")
    else
...
RegisterNetEvent('qb-phone:client:TransferMoney', function(amount, newmoney)
    ...
    SendNUIMessage({ action = "PhoneNotification", PhoneNotify = { title = "Aendir Bank", text = "&#36;"..amount.." Hesabınıza Eklendi!", icon = "fas fa-university", color = "#8c7ae6", }, })
...
RegisterNetEvent('qb-phone:client:NewMailNotify', function(MailData)
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Mail",
            text = "Yeni Bir Mail Geldi "..MailData.sender,
...
RegisterNetEvent('qb-phone:client:UpdateAdverts', function(Adverts, LastAd)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Bilgilendirme",
            text = "Yeni Bir İlan Verildi "..LastAd,
            icon = "fas fa-ad",
            color = "#ff8f1a",
            timeout = 2500,
        },
...
RegisterNetEvent('qb-phone:client:BillingEmail', function(data, paid, name)
    if paid then
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Ödeme Departmanı',
            subject = 'Fatura Ödendi',
            message = 'Fatura Ödendi: '..name..' Miktarı $'..data.amount,
        })
    else
        TriggerServerEvent('qb-phone:server:sendNewMail', {
            sender = 'Ödeme Departmanı',
            subject = 'Fatura Reddedildi',
            message = 'Fatura Ödenemedi: '..name..' Miktarı $'..data.amount,
        })
    end
end)
        ...
        if PhoneData.isOpen then
            if SenderNumber ~= PhoneData.PlayerData.charinfo.phone then
                SendNUIMessage({
                    ...
                    PhoneNotify = {
                        title = "Whatsapp",
                        text = "Yeni Mesaj Geldi: "..IsNumberInContacts(SenderNumber).."!",
                        icon = "fab fa-whatsapp",
                        color = "#25D366",
                        timeout = 1500,
                    },
...
RegisterNetEvent('qb-phone:client:RemoveBankMoney', function(amount)
    if amount > 0 then
        SendNUIMessage({
            ...
            PhoneNotify = {
                title = "Bank",
                text = "$"..amount.." Bakiyenizden Çekildi!",
                icon = "fas fa-university",
                color = "#ff002f",
                timeout = 3500,
            },
...
RegisterNetEvent('qb-phone:client:AddNewSuggestion', function(SuggestionData)
    ...
    SendNUIMessage({
        ...
        PhoneNotify = {
            title = "Telefon",
            text = "Önerilen yeni bir kişiniz var!",
            icon = "fa fa-phone-alt",
            color = "#04b543",
            timeout = 1500,
        },
